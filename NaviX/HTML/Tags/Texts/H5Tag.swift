...
struct H5Tag: BodyTagProtocol {
    ...
    var body: some View {
        Text(_children.joined(separator: " "))
            ...
            .modifier(CSSRuleSet.CSSFontModifier(ruleSet: style, defaultFontSize: 16, defaultFontWeight: .bold))
            .modifier(CSSRuleSet.CSSColorModifier(ruleSet: style))
            .applyCommonCSS(ruleSet: style, tag: self)
    }

    static func parse(_ elem: Element, html: @escaping HTMLGetter) throws -> H5Tag {
