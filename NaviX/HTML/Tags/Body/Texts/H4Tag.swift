...
final class H4Tag: BodyTagProtocol {
    let html: HTMLTag
    let tagName = "h4"
    let id = UUID()
    ...
    static func parse(_ elem: Element, html: HTMLTag) throws -> H4Tag {
...
struct H4TagView: View {
    @ObservedObject var tag: H4Tag
    
    var body: some View {
        Text(tag._children.joined(separator: " "))
            .textSelection(.enabled)
            .modifier(CSSRuleSet.CSSFontModifier(ruleSet: tag.style, defaultFontSize: 18, defaultFontWeight: .bold))
