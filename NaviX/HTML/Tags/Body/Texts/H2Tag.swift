...
final class H2Tag: BodyTagProtocol {
    let html: HTMLTag
    let tagName = "h2"
    let id = UUID()
    ...
    static func parse(_ elem: Element, html: HTMLTag) throws -> H2Tag {
        try HTMLUtils.checkTag(elem, assert: "h2")
        let (attr, children) = Self.parseDefaultProps(elem, html: html)

        return H2Tag(html: html, attr: attr, children: children)
    }
}

struct H2TagView: View {
    @ObservedObject var tag: H2Tag
    
    var body: some View {
        Text(tag._children.joined(separator: " "))
            .textSelection(.enabled)
            .modifier(CSSRuleSet.CSSFontModifier(ruleSet: tag.style, defaultFontSize: 22, defaultFontWeight: .bold))
