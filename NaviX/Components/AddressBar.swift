...
import SwiftUI
    ...
    class AddressToolbar: NSToolbar, NSToolbarDelegate, NSToolbarItemValidation {
        ...
        init(
            identifier: NSToolbar.Identifier, addressBarView: NSView,
            enableBackwardButton: Binding<Bool>, enableForwardButton: Binding<Bool>,
            addTabPressed: @escaping () -> Void, backwardPressed: @escaping () -> Void,
            forwardPressed: @escaping () -> Void
        ) {
        ...
        func toolbar(
            ...
            if itemIdentifier == .addressBar {
                ...
                toolbarItem.image = NSImage(
                    systemSymbolName: "chevron.right", accessibilityDescription: "Forward")
                ...
                toolbarItem.image = NSImage(
                    systemSymbolName: "chevron.left", accessibilityDescription: "Backward")
        ...
        func toolbarAllowedItemIdentifiers(_ toolbar: NSToolbar) -> [NSToolbarItem.Identifier] {
            return [
                .toggleSidebar, .flexibleSpace, .addTab, .sidebarTrackingSeparator, .backward,
                .forward, .flexibleSpace, .addressBar, .flexibleSpace,
            ]
        }
    ...
    class SearchBarContainer {
        ...
        init(
            enableBackwardButton: Binding<Bool>, enableForwardButton: Binding<Bool>,
            textBinding: Binding<String>, onSubmit: @escaping () -> Void,
            backwardPressed: @escaping () -> Void, forwardPressed: @escaping () -> Void
        ) {
            self.enableForwardButton = enableForwardButton
            self.enableBackwardButton = enableBackwardButton
            ...
            override func viewDidAppear(_ animated: Bool) {
                ...
                if let navigationController {
                    ...
                    backwardButton = UIBarButtonItem(
                        title: "Back", image: UIImage(systemName: "chevron.left"),
                        target: container, action: #selector(container.backwardAction(_:)))
                    forwardButton = UIBarButtonItem(
                        title: "Forward", image: UIImage(systemName: "chevron.right"),
                        target: container, action: #selector(container.forwardAction(_:)))
                    toolbarItems = [
                        backwardButton,
                        flexibleSpace,
                        forwardButton,
                        ...
                        flexibleSpace,
                        UIBarButtonItem(
                            title: "Tabs", image: UIImage(systemName: "square.grid.2x2"),
                            target: self, action: #selector(goBackToTabListPressed)),
                    ]
                    ...
                    bar.topItem?.titleView = UIView(
                        frame: CGRect(x: 0, y: 0, width: bar.frame.width, height: 44))
                    bar.topItem?.titleView?.addSubview(searchController.searchBar)

                    NSLayoutConstraint.activate([
                        searchController.searchBar.leadingAnchor.constraint(
                            equalTo: bar.topItem!.titleView!.leadingAnchor),
                        searchController.searchBar.trailingAnchor.constraint(
                            equalTo: bar.topItem!.titleView!.trailingAnchor),
                        searchController.searchBar.topAnchor.constraint(
                            equalTo: bar.topItem!.titleView!.topAnchor),
                        searchController.searchBar.bottomAnchor.constraint(
                            equalTo: bar.topItem!.titleView!.bottomAnchor),
                    ])
